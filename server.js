...
function serializeHostQuestion(question) {
...
function serializePlayerQuestion(question) {
  if (!question) return null;
  return {
    id: question.id,
    order: question.order,
...
function serializeMonitorQuestion(room) {
  ...
  return {
    ...
    order: question.order,
    questionText: question.questionText,
    answerText: normalizeRoomPhase(room.phase) === 'revealResults' ? question.answerText : '',
    answerMode: question.answerMode,
  };
}
...
function serializeHostRoom(room) {
  ...
  return {
    ...
    currentQuestion: serializeHostQuestion(room.currentQuestion),
...
function serializeMonitorRoom(room) {
  ...
  return {
    ...
    phase,
    answerMode: room.answerMode,
    currentQuestion: serializeMonitorQuestion(room),
    inputEnabled: phaseAllowsInput(phase),
    revealMode: phaseToRevealMode(phase),
...
function clearRound(room, nextPhase = 'setup') {
  ...
  room.answerMode = normalizedNextPhase === 'setup' ? 'text' : preservedAnswerMode;
  if (normalizedNextPhase === 'setup') {
...
app.get('/api/version', (_req, res) => {
  res.json({
    version: 'quiz-panel-questionboard-2026-05-02',
  });
});
...
io.on('connection', (socket) => {
  ...
  socket.on('host:setQuestion', ({ roomCode, question }, ack = () => {}) => {
    const room = requireHost(roomCode, socket, ack);
    if (!room) return;

    if (normalizeRoomPhase(room.phase) !== 'setup') {
      ack({ ok: false, message: '問題は準備中だけセットできます。' });
      return;
    }

    const normalizedQuestionText = normalizeQuestionText(question?.questionText);
    const normalizedAnswerText = normalizeQuestionText(question?.answerText);
    const normalizedAnswerMode = parseRoomAnswerMode(question?.answerMode)
      || parseRoomAnswerMode(room.answerMode)
      || 'text';

    if (!normalizedQuestionText) {
      ack({ ok: false, message: '問題文をセットしてください。' });
      return;
    }

    room.currentQuestion = {
      id: String(question?.id || '').trim() || `question-${Date.now()}`,
      order: Number(question?.order) || 0,
      questionText: normalizedQuestionText,
      answerText: normalizedAnswerText,
      answerMode: normalizedAnswerMode,
      note: normalizeQuestionNote(question?.note),
    };
    room.answerMode = normalizedAnswerMode;
  ...
  socket.on('host:setPhase', ({ roomCode, phase }, ack = () => {}) => {
    ...
    if (nextPhase === 'open') {
      ...
      if (!room.currentQuestion?.questionText) {
        ack({ ok: false, message: '先に問題をセットしてください。' });
        return;
      }
      if (!room.answerMode) {
